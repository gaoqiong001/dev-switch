...
export default function LanguagesPage({
  ...
  const [modalState, setModalState] = useState<{
    ...
    name: string;
    guide?: LanguageInfo['install_guide'] | string;
  ...
  if (data.length === 0) return <EmptyState icon="🌐" title={t('empty.nothingDetectedLanguages')} />;
      ...
      <div className="grid grid-cols-[repeat(auto-fill,minmax(300px,1fr))] gap-5">
        {data.map((lang) => (
          <GlassCard
            key={lang.name}
            icon={lang.installed ? '🌐' : '⚪'}
            title={<Highlight text={lang.name} query={searchQuery} />}
            unavailable={!lang.installed}
            actions={
              lang.installed ? (
                <button
                  onClick={() => setModalState({ isOpen: true, type: 'uninstall', name: lang.name, guide: lang.uninstall_guide })}
                ...
                <button
                  onClick={() => setModalState({ isOpen: true, type: 'install', name: lang.name, guide: lang.install_guide })}
          ...
          >
            <InfoRow label={t('cards.version')} value={lang.version || t('cards.notInstalled')} />
            {lang.path && <InfoRow label={t('cards.path')} value={lang.path} mono />}
            <div className="flex justify-between items-center py-2.5">
              <span className="text-[13px] text-[var(--muted-fg)] font-medium">{t('cards.status')}</span>
              <StatusBadge installed={lang.installed} />
            </div>
          </GlassCard>
        ))}
      </div>
